...
static regmatch_t	re_match_[RE_NMATCH];
...
retry:
		...
		switch (getkey(FALSE)) {
		case ' ':
			plen = re_match_[0].rm_eo - re_match_[0].rm_so;
	...
	while (more) {
		...
		case 2:
			if (*st >= '0' && *st <= '9') {
				...
				if (num >= RE_NMATCH)
					return (FALSE);
				k = re_match_[num].rm_eo - re_match_[num].rm_so;
				if (j + k >= REPLEN)
					return (FALSE);
				bcopy(&(clp->l_text[re_match_[num].rm_so]),
				    &repstr[j], k);
				j += k;
				if (*st == '\0')
					more = FALSE;
...
re_forwsrch(void)
{
	...
	while (clp != (curbp->b_headp)) {
		re_match_[0].rm_so = tbo;
		re_match_[0].rm_eo = llength(clp);
		error = regexec(&re_buff, ltext(clp), RE_NMATCH, re_match_,
		    REG_STARTEND);
		if (error != 0) {
			clp = lforw(clp);
			tbo = 0;
		} else {
			curwp->w_doto = re_match_[0].rm_eo;
			curwp->w_dotp = clp;
...
re_backsrch(void)
{
	...
	while (clp != (curbp->b_headp)) {
		 ...
		 * non-match does not modify the re_match_ array.  We have to
		 * do this character-by-character after the first match since
		 * POSIX regexps don't give you a way to do reverse matches.
		 */
		while (!regexec(&re_buff, ltext(clp), RE_NMATCH, re_match_,
		    REG_STARTEND) && re_match_[0].rm_so < tbo) {
			memcpy(&lastmatch, &re_match_[0], sizeof(regmatch_t));
			re_match_[0].rm_so++;
			re_match_[0].rm_eo = llength(clp);
		}
		if (lastmatch.rm_so == -1) {
			...
			memcpy(&re_match_[0], &lastmatch, sizeof(regmatch_t));
			curwp->w_doto = re_match_[0].rm_so;
			curwp->w_dotp = clp;
			curwp->w_rflag |= WFMOVE;
			return (TRUE);
		}
